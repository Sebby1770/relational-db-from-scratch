...
This repository now includes a broad implementation pass across the full roadmap, plus the 0.3.0 query surface (2026-08-18):
...
- Week 6: `COUNT(*)`, `SUM`, `GROUP BY` hash aggregation, `ORDER BY`, `LIMIT n OFFSET m`, `LIKE`, and CSV `COPY` / `.import`.
...
Still next after 0.3.0: hash join, `HAVING`, cost-based join order, and on-disk table files.
